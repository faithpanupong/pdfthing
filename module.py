...
# acc = False
def rename_file(string_data,DEBUGRENAME = False):
    acc = False
    if DEBUGRENAME == False:
    ...
    file_name = ''.join(['N ',ntype, nregis_number, nagency, nlocation])
    return (file_name,acc)

def is_filename_new(filename, DEBUGFILENAME):
    if DEBUGFILENAME: return True
    if filename[:-9].isnumeric():
        return True
    return False

def start(folder_path, stop=-1, DEBUGRENAME=False, DEBUGFILENAME=False):
  count = 0
  #start
  ...
  for filename in files:
    ...
    if not is_filename_new(filename, DEBUGFILENAME):
